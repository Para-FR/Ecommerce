...
<div class="banner-top">
	<div class="container">
		<h1>Recherche</h1>
		<em></em>
		<h2><a href="index.php">Accueil</a><label>/</label>Recherche</h2>
	</div>
</div>
	<!--content-->
		<div class="product">
			...
			<div class="col-md-10">
				<?php
				if (isset($recherche)) {
					executeRequete("INSERT INTO recherche(recherche_client) VALUES ('$recherche')");
					echo '<h1 class="text-center">Resultat de la Recherche : '. $recherche .'</h1>
				<hr>';
				$collec_femme = executeRequete("SELECT * FROM produit where titre like '%".$recherche."%'
												OR description like '%".$recherche."%'
												OR couleur like '%".$recherche."%'
												OR categorie like '%".$recherche."%'");

					if ($collec_femme->num_rows > 0){
						foreach ($collec_femme as $produit => $value){
							if ($value['public'] == 'f'){
								$public = '<span>Femme</span>';
							}else{
					...
					<div class="pro-img">
						...
						echo '<p class="text-center">Aucun produit n\' été trouvé</p>';
					}
